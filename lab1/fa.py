...
class Automata(object):
    ...
    def visualize(self, filename):
        ...
        for state in states:
            for code, char_transition in state.charTransitions.items():
                for dest in char_transition:
                    fa_graph.edge(str(state), str(dest), label=chr(code))
            for dest in state.epsilonTransitions:
                fa_graph.edge(str(state), str(dest), label='eps')
    ...
    def states(self):
        ...
        while len(state_stack) != 0:
            ...
            if state not in visited:
                ...
                for d in state.charTransitions.keys():
                    for destinations in state.charTransitions[d]:
                        if destinations is not None and destinations not in visited:
                            state_stack.append(destinations)
                for destinations in state.epsilonTransitions:
                    if destinations is not None and destinations not in visited:
                        state_stack.append(destinations)
